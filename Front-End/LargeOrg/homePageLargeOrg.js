...
//buttons
const chooseBasicBtn = document.getElementById("choosselect");
...
const showContactManAssociationBtn = document.getElementById('showcontactmansssociation-btn_id');
...
const showDetailsAssociationModal= document.getElementById('showdetailsassociationmodal_id');
...
//
showContactManAssociationBtn.addEventListener("click", function() {
  event.preventDefault();

  if(flagDeleteOpen === 1 || flagSettingOpen){
    return;
  }
  const selectAssociation = document.querySelector('input[name="donationRadio"]:checked');
  if (selectAssociation) {
   const associationNumber = selectAssociation.value;
   //Update the donation count in the HTML
   const nameAssociation = tempAssociationName[associationNumber];
   const donationCountElement = document.getElementById('associationname_id');
   donationCountElement.textContent = `association- ${nameAssociation}`;
    closePopupWindow();
    displayContactMenAssociation(associationNumber);
    openShowAllContactManAssociationModal();
  } else {
      console.log('No association selected.');
      alert("Please select a association to view the data");
  }
});
//
...
chooseBasicBtn.addEventListener("click", function() {
    ...
    switch (basicInput) {
        case "editprofile":
            window.location.href = "editUserLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "settinguser":
            ...
            break;
        case "addcontactman":
            window.location.href = "addContactManLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "updatecontactman":
            window.location.href = "upDateContactmanLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "deletacontactman":
            window.location.href = "deleteContactManLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "deletAccount":
              closeModals();
              flagDeleteOpen = 1;
...
chooseActionBtn.addEventListener("click", function() {
    ...
    switch (actionInput) {
        case "addonation":
            window.location.href = "addNewDonationLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "updatedonation":
            window.location.href = "updateDonationLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        case "deletedonation":
            window.location.href = "deleteDonationLargeOrg.html?username=" + encodeURIComponent(username);
            break;
        default:
            break;
    }
...
function displayAllAssociation() {
  ...
  fetch(`http://localhost:3001/getUsersByType?type=${encodeURIComponent(type)}`)
    .then(response => response.json())
    .then(data => {
      const tableBody = document.getElementById('tableAssociationshow_body');

      // Clear existing table body content
      tableBody.innerHTML = '';
      if (Array.isArray(data.users)) {
        // Create table rows and populate with data
        data.users.forEach(userAssociation => {
          const row = tableBody.insertRow();
          const radioCell = row.insertCell();
          ...
          nameCell.textContent = userAssociation.name;
          emailCell.textContent = userAssociation.email;
          phoneCell.textContent = userAssociation.phone;
          addressCell.textContent = userAssociation.address +" "+ userAssociation.city;
       
          tempAssociationEmail[i] = userAssociation.email;
          tempAssociationName[i] = userAssociation.name;
          i++;
...
//
function displayContactMenAssociation(index) {
  ...
  fetch(`http://localhost:3001/getAllContactMan?email=${encodeURIComponent(email)}`)
    .then(response => response.json())
    .then(data => {
      const tableBody = document.getElementById('tableassociationscontactmanshow_body');
