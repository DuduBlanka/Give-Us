...
function checkInput() {
  ...
  const emailField = document.getElementById("email_id").value.trim();
  const repeatEmailField = document.getElementById("email-repeat_id").value.trim();
...
// Define regex patterns for email and phone validation
  ...
  // Check if all required fields are filled in
    if (emailField === "" && repeatEmailField === "" &&
        phoneNumberField.value === "" && repeatPhoneNumberField === "" && addressField === "" &&
        cityField === "" && passwordField === "" && repeatPasswordField === "") {
        // Display a warning message
      alert("What details would you like to change?");
      return false;
    }
    if ((emailField === "" && repeatEmailField !== "") || (emailField !== "" && repeatEmailField === "")){
      alert("Please fill in both email fields.");
      return false;
    }
  
    if ((phoneNumberField === "" && repeatPhoneNumberField !== "") || (phoneNumberField !== "" && repeatPhoneNumberField === "")) {
      alert("Please fill in both phone number fields.");
      return false;
    }
  
    if ((passwordField === "" && repeatPasswordField !== "") ||(passwordField !== "" && repeatPasswordField === "")) {
    ...
    if(phoneNumberField !== ""){
      ...
      if (phoneNumberField !== repeatPhoneNumberField) {
          // Display a warning message
...
function sendDetailsToDatabase() {
  ...
  fetch(`http://localhost:3001/getUsers?email=${encodeURIComponent(email)}`)
    ...
    .then((data) => {
      ...
      oldEmail = email;
      newEmail = document.getElementById("email_id").value.trim();
      if (newEmail !== '') {
        email = newEmail;
      }
      else{
      ...
      if (newCity !== '') {
          city = capitalizeFirstLetter(newCity.trim());
      }
